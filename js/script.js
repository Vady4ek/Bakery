...
let firstSlider = $('.recipe-body-slider__body')
...
$('.star').raty({
    ...
    score: 4,
    start: 1, // Отчет со значения.
    starOff: 'star-off.png', // Имя выключенной звезды
    starOn: 'star.png' // Имя включенной звезды.
...
//==================================================================================================
function adaptiveMove(moveWidth) {
    let initialContainer = $('.header__row')
    let moveContainer = $('.menu__list');
    let moveElement = $('.header__social');
    if (moveWidth <= 850) {
        if (!($(moveElement).hasClass('moved'))) {
            $(moveElement).appendTo(moveContainer);
            $(moveElement).wrap('<li></li>');
            $(moveElement).addClass('moved');
        }
    } else {
        if ($(moveElement).hasClass('moved')) {
            $(moveElement).unwrap();
            $(moveElement).appendTo(initialContainer);
            $(moveElement).removeClass('moved');
        }
    }
}
//==================================================================================================
